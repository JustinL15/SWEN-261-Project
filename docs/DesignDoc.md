...
Every user story within sprint 2 passes its acceptance criteria. Each team member was given 2-4 user stories to complete and acceptance criteria for each was determined as a team. During pull requests, team members reviewed the code and the user stories defined and determined whether that user story was implemented successfully before a merge was accepted. The main issues that occurred with acceptance criteria was that for some user stories, the implementation was more difficult, making the criteria harder to reach. For example, Write Reviews involved of a significant number of checks to ensure a customer has purchased a product in order to review it, retrieves a customer's previous review to be edited instead of creating a new review, and ensures the rating is valid. It also utilizes multiple asyncronous and await calls to ensure the correct data is being utilized to create and display a new review. However, whenever a teammate had an issue or a question, there was never hesitation to ask for help and all user stories were able to fully meet their acceptance criteria by the end of the sprint.
